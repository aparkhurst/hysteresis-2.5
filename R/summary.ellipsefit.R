summary.ellipsefit <-
function(object,boot=TRUE,N=1000,studentize=TRUE,center=FALSE,cbb=NULL,joint=FALSE,seed=NULL,repeated=TRUE,...) {
if (!is.null(seed)) set.seed(seed)
thecall <- match.call()
if (boot==TRUE) {
  if (object$method=="harmonic2")
  {res <- harmonic2summary(object,N=N,studentize=studentize,cbb=cbb,joint=joint,repeated=repeated)}
